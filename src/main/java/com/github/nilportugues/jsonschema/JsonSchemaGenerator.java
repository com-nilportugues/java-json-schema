...
package com.github.nilportugues.jsonschema;
...
import com.github.nilportugues.jsonschema.annotation.JsonSchema;
import com.github.nilportugues.jsonschema.annotation.Nullable;
import com.github.nilportugues.jsonschema.annotation.SchemaIgnore;
import com.github.nilportugues.jsonschema.exception.TypeException;
...
public abstract class JsonSchemaGenerator {
     ...
     * @param props
     */
    protected abstract void processSchemaProperty(ObjectNode schema, JsonSchema props);
    ...
    private <T> void processCustomCollection(Class<T> type, ObjectNode schema) throws TypeException {
        schema.put(TAG_TYPE, TAG_ARRAY);
        final Field field = type.getDeclaredFields()[0];

        final ParameterizedType genericType = (ParameterizedType) field.getGenericType();

        final Class<?> genericClass = (Class<?>) genericType.getActualTypeArguments()[0];

        final ObjectNode itemsSchema = generateSchema(genericClass);
    ...
    protected <T> void processRootAttributes(Class<T> type, ObjectNode schema) {
        JsonSchema sProp = type.getAnnotation(JsonSchema.class);
        if (sProp != null)
    ...
    protected <T> ObjectNode generatePropertySchema(Class<T> type, Method method, Field field) throws TypeException {
        ...
        JsonSchema attrs = propertyReflection.getAnnotation(JsonSchema.class);
        if (attrs != null) {
        ...
        for (Method method : methods) {
            ...
            if (isGetter(method)) {
                ...
                for (Field field : fields) {
                    ...
                    JsonSchema attribs = field.getAnnotation(JsonSchema.class);
    ...
    private <T> List<Field> findFields(Class<T> type) {
        ...
        for (Field field : fields) {
            ...
            if (field.getName().equalsIgnoreCase(name)) {
                JsonSchema attrs = field.getAnnotation(JsonSchema.class);
